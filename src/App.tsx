...
function App() {
    return (
        ...
        网格布局响应式：
        - 基础 (<576px): 单列 (1fr)
        - lg (≥1024px): 两列 (240px 占满剩余)
        - desktop (≥1440px): 左侧边栏加宽至 280px
      */}
            <div className="grid min-h-[calc(100vh-2rem)] grid-cols-1 gap-4 xs:gap-6 lg:grid-cols-[240px_1fr] desktop:grid-cols-[280px_1fr] desktop:gap-8">
                ...
                {/* ===== 导航栏：始终横跨所有列 ===== */}
                <header className="flex h-16 items-center justify-between rounded-lg border border-border bg-card px-4 shadow-sm xs:px-6 lg:col-span-2">
                    ...
                    <nav className="flex gap-3 text-xs text-muted-foreground xs:gap-4 xs:text-sm">
                        <span>Dashboard</span>
                        <span className="hidden xs:inline">Settings</span> {/* 576px 以上才显示 */}
              ...
              xs: 576px &nbsp;|&nbsp; lg: 1024px &nbsp;|&nbsp; desktop: 1440px
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-3 xs:grid-cols-2 desktop:grid-cols-3">
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="rounded-md bg-muted/50 p-4 text-center xs:text-left">
